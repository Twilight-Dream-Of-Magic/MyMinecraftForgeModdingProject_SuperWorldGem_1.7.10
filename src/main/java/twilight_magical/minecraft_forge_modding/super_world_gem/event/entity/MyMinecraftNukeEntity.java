...
public class MyMinecraftNukeEntity extends Entity {
	...
	public MyMinecraftNukeEntity(World thisWorldObj) {
		super(thisWorldObj);
		this.fuse = LoadUnstableNuke.myNukeFuseTime;
	...
	public MyMinecraftNukeEntity(World thisWorldObj, double positionX, double positionY, double positionZ, EntityLivingBase entity_living_base) {
		...
		this.tntPlacedBy = entity_living_base;
	}
	...
	public MyMinecraftNukeExplosion newExplosion(Entity entity, double positionX, double positionY, double positionZ, float myNukeExplodeSize, boolean flagFlaming, boolean flagSmoking)
	{
		...
		while (iterator.hasNext()) 
		{
			...
			{
				( (EntityPlayerMP) entityplayer).playerNetServerHandler.sendPacket((Packet)new S27PacketExplosion(positionX, positionY, positionZ, myNukeExplodeSize, explosion.affectedBlockPositions, (Vec3)explosion.getHashMap().get(entityplayer)));
			}
		} 
		return explosion;
	}
	...
	private void explode() {
		float explosionForce = LoadUnstableNuke.myNukeExplodeForce;
		this.createExplosion(this, this.posX, this.posY, this.posZ, explosionForce, true);
	}
	...
	{
		par1NBTTagCompound.setByte("FuseTime", (byte)this.fuse);
	}
	
	@Override
	protected void readEntityFromNBT(NBTTagCompound par1NBTTagCompound)
	{
		this.fuse = par1NBTTagCompound.getByte("FuseTime");
	}
